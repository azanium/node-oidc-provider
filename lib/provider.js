...
class Provider extends events.EventEmitter {
  constructor(issuer, setup = {}) {
    ...
    this.clientClearCache = () => models.getClient(this).cacheClear();

    // Generate Admin token client credentials
    this.generateAdminAccessToken = async (client, scope, perms) => {
      const ClientCredentials = models.getClientCredentials(this);
