...
<body>
   ...
   * permissions
   *
   * description: Helper used by the OP to push additional permissions to issued ID, Access and
   *   ClientCredentials Tokens as well as other signed responses (e.g. userinfo). The return value
   *   should either be falsy to omit adding additional permissions or an array of strings to push.
   * affects: ID Token permissions, access token permissions, client credential permissions, signed
   *   UserInfo permissions
   */
  async permissions(ctx, sub, token, use) { // eslint-disable-line no-unused-vars
    // @param ctx   - koa request context
    // @param sub   - account identifier (subject)
    // @param token - the token to which these additional audiences will be assed to. It is
    //   undefined when the audiences are pushed to a JWT userinfo response
